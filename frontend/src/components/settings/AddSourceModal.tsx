...
export default function AddSourceModal({ onClose }: Props) {
  ...
  const [form, setForm] = useState({ name: '', url: '', feed_type: 'rss', trust_weight: 1.0 })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-stone-900 border border-stone-700 rounded-2xl p-6 w-full max-w-md space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-stone-500 hover:text-stone-200"><X className="w-4 h-4" /></button>
        </div>
        ...
        <div>
          ...
          <input
            type="range" min={0.5} max={2.0} step={0.1} value={form.trust_weight}
            onChange={(e) => setForm((f) => ({ ...f, trust_weight: Number(e.target.value) }))}
