...
export default function VerticalDetail({ verticalId }: { verticalId: number }) {
  ...
  const { data: vertical, isLoading } = useQuery({
    queryKey: ['verticals', verticalId],
    queryFn: () => fetchVertical(verticalId),
  })
  const { data: newsData } = useQuery({
    queryKey: ['verticals', verticalId, 'news'],
    queryFn: () => fetchVerticalNews(verticalId),
    enabled: !!vertical,
  })
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <aside className="relative w-full max-w-2xl bg-stone-950 border-l border-stone-800 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-stone-800">
          <span className="text-sm font-semibold text-stone-300">Vertical</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          {vertical && (
            <>
              <div>
                <h2 className="text-xl font-bold text-blue-400">{vertical.name}</h2>
