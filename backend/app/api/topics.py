...
async def create_topic_endpoint(body: TopicCreate, db: AsyncSession = Depends(get_db)):
    ...
    seeded_sources_count, seed_status, seed_message = await seed_sources_for_topic(db, get_llm_service(), topic)
...
async def update_topic_endpoint(topic_id: int, body: TopicUpdate, db: AsyncSession = Depends(get_db)):
...
async def delete_topic_endpoint(topic_id: int, db: AsyncSession = Depends(get_db)):
    topic = await get_topic(db, topic_id)
    if not topic:
        raise HTTPException(status_code=404, detail="Topic not found")
    if topic.is_default:
